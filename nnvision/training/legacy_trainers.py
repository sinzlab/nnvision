...
from neuralpredictors.measures.functions import *
...
def early_stop_trainer(
    ...
    **kwargs
    ...
    def full_objective(model, data_key, inputs, targets, **kwargs):
        ...
        return np.sqrt(m / k) * criterion(
            model(inputs.to(device), data_key=data_key, **kwargs), targets.to(device)
        ).sum() + model.regularizer(data_key)
    ...
    def run(
        ...
        for epoch, val_obj in early_stopping(
            ...
            for batch_no, (data_key, data) in tqdm(
                enumerate(LongCycler(train_loader)), desc="Epoch {}".format(epoch)
            ):
    ...
    **kwargs
    ...
    def full_objective(model, data_key, inputs, targets):
        ...
        return loss_scale * criterion(
            model(inputs.to(device), data_key), targets.to(device)
        ) + model.regularizer(data_key)

    trainloaders = dataloaders["train"]
    valloaders = dataloaders.get(
        "validation", dataloaders["val"] if "val" in dataloaders.keys() else None
    )
    ...
    stop_closure = partial(
        getattr(measures, stop_function), model, valloaders, device=device
    )
    ...
    tracker_dict = dict(
        correlation=partial(corr_stop, model, valloaders, device=device),
