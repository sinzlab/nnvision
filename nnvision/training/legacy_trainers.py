...
def early_stop_trainer(model, seed, stop_function='get_correlations',
    ...
    def full_objective(model, data_key, inputs, targets, **kwargs):
        ...
        return np.sqrt(m / k) * criterion(model(inputs.to(device), data_key=data_key, **kwargs),
                                          targets.to(device)).sum() \
...
def standard_early_stop_trainer(model, dataloaders, seed, avg_loss=True, scale_loss=True,  # trainer args
    ...
    def full_objective(model, data_key, inputs, targets):
        ...
        return loss_scale * criterion(model(inputs.to(device), data_key), targets.to(device)) + model.regularizer(
            data_key)
    ...
    stop_closure = partial(getattr(measures, stop_function), model, valloaders, device=device)
    ...
    for epoch, val_obj in early_stopping(model, stop_closure, interval=interval, patience=patience,
        ...
        # train over batches
        optimizer.zero_grad()
        for batch_no, (data_key, data) in tqdm(enumerate(LongCycler(trainloaders)), total=n_iterations,
                                               desc="Epoch {}".format(epoch)):
