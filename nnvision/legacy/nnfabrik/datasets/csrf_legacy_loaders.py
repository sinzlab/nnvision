...
def csrf_v1_legacy(
    ...
    train_loader = get_loader_csrf_V1_legacy(
        images, responses, 1 * valid_responses, batch_size=batch_size
    )

    images, responses, valid_responses = v1_data.val()
    val_loader = get_loader_csrf_V1_legacy(
        images, responses, 1 * valid_responses, batch_size=batch_size
    )
...
def get_loader_csrf_V1_legacy(
    images, responses, valid_responses, batch_size=None, shuffle=True, retina_warp=False
):
...
class CSRF_V1_Data:
    ...
    def __init__(
        ...
        images_train = raw_data["images_train"][
            :, crop : h - crop : subsample, crop : w - crop : subsample
        ]
        images_test = raw_data["images_test"][
            :, crop : h - crop : subsample, crop : w - crop : subsample
        ]
        ...
        train_split, val_split = self.get_validation_split(
            real_responses, train_frac, seed
        )
        ...
        if time_bins_sum is not None:  # then average over given time bins
            self._responses_train = np.sum(
                self._responses_train[:, :, time_bins_sum], axis=-1
            )
            self._responses_test = np.sum(
                self._responses_test[:, :, time_bins_sum], axis=-1
            )
            self._responses_val = np.sum(
                self._responses_val[:, :, time_bins_sum], axis=-1
            )

            # In real responses: If an entry for any time is False, real_responses is False for all times.
            self._real_responses_train = np.all(
                self._real_responses_train[:, :, time_bins_sum], axis=-1
            )
            self._real_responses_test = np.all(
                self._real_responses_test[:, :, time_bins_sum], axis=-1
            )
            self._real_responses_val = np.all(
                self._real_responses_val[:, :, time_bins_sum], axis=-1
            )
    ...
    def get_validation_split(self, real_responses_train, train_frac=0.8, seed=None):
        ...
        top_images = Neurons_per_image_sort_idx[
            -int(np.floor(train_frac / 2 * num_images)) :
        ]
        val_images_idx = np.random.choice(
            top_images, int(len(top_images) / 2), replace=False
        )

        train_idx_filter = np.logical_not(
            np.isin(Neurons_per_image_sort_idx, val_images_idx)
        )
        train_images_idx = np.random.permutation(
            Neurons_per_image_sort_idx[train_idx_filter]
        )
