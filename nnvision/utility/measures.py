            ...
            with eval_state(model) if not is_ensemble_function(
                ...
                with device_state(model, device) if not is_ensemble_function(
                    ...
                    if mask_flag:  # apply mask to output
                        batch_output = np.ma.masked_array(
                            batch_output, mask=~batch_mask
                        )
...
def model_predictions_shuffled(
    ...
    Returns:
        ...
        zeroed_out: Boolean, whether to input zeros rather than targets for context modulator
    """
    batch = next(iter(dataloader))
    if (
...
def model_predictions_mean_input(
    model,
    dataloader,
    data_key,
    device="cuda",
            ...
            with device_state(model, device) if not is_ensemble_function(
                ...
                with torch.no_grad():
                    output = torch.cat(
                        ...
                        dim=0,
                    )  # get output as predicted by model
            target = torch.cat(
                (target, responses.detach().cpu()), dim=0
            )  # get responses from batch
            if mask_flag:
                masks = torch.cat((masks, mask))
...
def get_correlations_mean_input(
    ...
    with eval_state(model) if not is_ensemble_function(
        ...
        for k, v in dataloaders.items():
            print(k)
            target, output = model_predictions_mean_input(
                dataloader=v,
                model=model,
                data_key=k,
                device=device,
