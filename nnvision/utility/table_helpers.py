...
def fill_augmented_model_tables(
    trainedmodel_keys,
    model_fn=None,
    model_config=None,
    model_table=None,
    ...
    for attribute in ["dataset_hash", "model_hash", "trainer_hash"]:
        if len(np.unique([key[attribute] for key in trainedmodel_keys])) != 1:
            raise ValueError(
                f"trainedmodel_keys are only allowed to differ in seed. "
    ...
    for key in trainedmodel_keys:
        ...
        insert_key = model_table().add_entry(
            model_fn=model_fn, model_config=model_config, model_comment=model_comment
        )
    ...
    transfer_trainedmodel_table().populate(
        transfer_trainedmodel_table_keys, display_progress=True, reserve_jobs=True
    )
    ...
    if add_ensemble:
        ...
        transfer_ensemble_table().create_ensemble(
            key=transfer_trainedmodel_table_keys, comment=model_comment
        )

        ensemble_hash = (
            transfer_ensemble_table.Member() & transfer_trainedmodel_table_keys
        ).fetch("ensemble_hash", limit=1)[0]
