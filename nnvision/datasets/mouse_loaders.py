...
def mouse_static_loader(
    ...
    **kwargs
    ...
    for tier in keys:
        ...
        # sample images
        subset_idx = (
            np.where(dat.trial_info.tiers == tier)[0]
            if file_tree
            else np.where(dat.tiers == tier)[0]
        )
        sampler = (
...
def mouse_static_loaders(
    ...
    **kwargs
    ...
    for path, neuron_id in zip_longest(paths, neuron_ids, fillvalue=None):
        data_key, loaders = mouse_static_loader(
            path,
            batch_size,
            seed=seed,
            areas=areas,
...
def mouse_shared_static_loaders(
    paths,
    batch_size,
    seed=None,
    areas=None,
    ...
    **kwargs
    ...
    if len(paths) == 1:
        warnings.warn(
            "Only one dataset was specified in 'paths'. When using the 'mouse_shared_loaders', more than one dataset"
            "has to be passed. Returning Dataloaders as if the function 'mouse_static_loaders' has been called."
        )
        dls = mouse_static_loaders(
            paths=paths,
    ...
    if neuron_ids is None:
        ...
        neuron_ids = [
            list(set(unit_ids) & set(ni))
            for unit_ids, ni in zip(all_set_neurons, neuron_ids)
        ]
