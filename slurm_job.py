...
class SlurmJob:
    def __init__(
            self, name, time, gpu, num_gpus, memory, email, on_slurm, index=0,
    ):
...
echo $CUDA_VISIBLE_DEVICES
    ...
    def run(self):
        ...
        if self.on_slurm:
            ...
            slurm_job_bash_file_content = "#!/bin/bash \n \n" \
            ...
            try:
                print(subprocess.check_output("sbatch " + slurm_job_bash_file, shell=True))
            finally:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Running jobs on SLURM cluster"
    )
    parser.add_argument(
        "--njobs", dest="num_jobs", action="store", default=1, type=int, help="",
    )
    parser.add_argument(
        "--name", dest="name", action="store", default="noname", type=str, help="",
    )
    parser.add_argument(
        "--time", dest="time", action="store", default="0-00:00", type=str,
        help="time to complete each job. Specify in the following format: D-HH:MM",
    )
    parser.add_argument(
        "--gpu", dest="gpu", action="store", default="gpu-2080ti", type=str, help="",
    )
    parser.add_argument(
